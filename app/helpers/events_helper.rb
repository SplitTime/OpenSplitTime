module EventsHelper
  ...
  def link_to_event_reassign_join(event, desired_event_group)
    link_to fa_icon("sign-in-alt"),
            reassign_event_group_event_path(event.event_group, event,
                                            event: { event_group_id: desired_event_group.id }),
            data: {
              controller: :tooltip,
              bs_placement: "bottom",
  ...
  def results_template_selector(resource)
    ...
    grouped_collection_select(resource_type, :results_template_id, organizations, :results_templates, :name, :id, :name,
                              ...
                              { class: "form-control dropdown-select-field",
                                data: { "results-template-target" => "dropdown",
                                        action: "results-template#replaceCategories" } })
  end

  def link_to_beacon_button(view_object)
    return unless view_object.beacon_url
  ...
  def link_to_download_spread_csv(view_object, current_user)
    ...
    link_to fa_icon("file-csv", text: "Export CSV"),
            spread_event_path(view_object.event, format: :csv, display_style: view_object.display_style,
                                                 sort: view_object.sort_hash),
            class: "btn btn-md btn-success"
  end

  def suggested_match_id_hash(efforts)
    efforts.select(&:suggested_match).to_h { |effort| [effort.id, effort.suggested_match.id] }
  end
