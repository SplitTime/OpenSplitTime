module TabsHelper
  def effort_view_tabs(view_object)
    return unless !view_object.simple? || current_user&.authorized_to_edit?(view_object.effort)
    ...
    items = [{ name: "Split times",
             ...
             { name: "Places + peers",
               ...
               hidden: view_object.simple? || view_object.not_analyzable? },
             { name: "Crew Access",
               link: crew_access_effort_path(view_object.effort),
               active: action_name == "crew_access",
               hidden: !view_object.event.gating_location_events.exists? },
             { name: "Audit",
