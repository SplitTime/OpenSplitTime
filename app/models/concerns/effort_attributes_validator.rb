class EffortAttributesValidator < ActiveModel::Validator
  ...
  def validate_split_times(effort)
    ...
    if record_event_course && effort.split_times.eager_load(split: :course).any? do |st|
      st.split.course != record_event_course
    end
  ...
  def validate_bib_number(effort)
    ...
    return if conflicting_effort.blank?
  ...
  def validate_names_and_birthdates(effort)
    ...
    conflicting_effort = ::Effort.where(event: effort.events_within_group)
                                 .where(first_name: effort.first_name, last_name: effort.last_name,
                                        birthdate: effort.birthdate)
                                 .where.not(id: effort.id).order(:id).first

    return if conflicting_effort.blank?

    message = "#{effort.full_name} with birthdate #{effort.birthdate} already exists for " \
              "bib number #{conflicting_effort.bib_number} (id #{conflicting_effort.id})"
  ...
  def validate_person(effort)
    ...
    return unless conflicting_person
