module Matchable
  ...
  def possible_matching_people
    ...
    result = result.where.not(id: id) if is_a?(Person)
  ...
  # @return [Person, nil]
  # Suitable for automated matcher
  def exact_matching_person
  ...
  # For situations where middle names are sometimes included with first_name and sometimes with last_name
  def people_same_full_name
    Person.full_name_matches(full_name)
  end

  # To pick up discrepancies in first names #TODO use levensthein alagorithm
  def people_changed_first_name
    Person.last_name_matches_exact(last_name).gender_matches(gender).age_matches(current_age)
  end

  # To find women who may have changed their last names
  def people_changed_last_name
  ...
  # Need to find a good nickname gem
  def people_with_nickname
