module Searchable
  ...
  included do
    ...
    scope :last_name_matches_exact, -> (param) { where(arel_table["last_name"].matches(param.to_s)) }
    scope :full_name_matches, -> (param) { where("regexp_replace((first_name || last_name), '[^a-zA-Z0-9]+', '', 'g') ILIKE ?", normalize(param).to_s) }
  end
