class SplitAttributesValidator < ActiveModel::Validator
  def validate(split)
    validate_vert(split)
    validate_distance(split)
    validate_location(split)
    validate_order(split) if split.intermediate?
  end

  private

  def validate_vert(split)
    if split.start?
