class SplitAttributesValidator < ActiveModel::Validator
  ...
  def validate_vert
    if split.start?
      unless split.vert_gain_from_start.nil? || split.vert_gain_from_start.zero?
        split.errors.add(:vert_gain_from_start, "for the start split must be 0")
      end
      unless split.vert_loss_from_start.nil? || split.vert_loss_from_start.zero?
        split.errors.add(:vert_loss_from_start, "for the start split must be 0")
      end
    else
  ...
  def validate_distance
    if split.start?
      ...
      unless split.distance_from_start&.positive?
        split.errors.add(:distance_from_start, "must be positive for intermediate and finish splits")
      end
    end
  end

  def validate_location
    unless split.elevation.nil? || split.elevation.between?(-1000, 10_000)
      split.errors.add(:elevation, "must be between -1000 and 10,000 meters")
    end
    unless split.latitude.nil? || split.latitude.between?(-90, 90)
      split.errors.add(:latitude, "must be between -90 and 90")
    end
    unless split.longitude.nil? || split.longitude.between?(-180, 180)
      split.errors.add(:longitude, "must be between -180 and 180")
    end
  end
