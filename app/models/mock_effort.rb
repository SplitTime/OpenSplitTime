# frozen_string_literal: true

class MockEffort < EffortWithLapSplitRows
  attr_reader :event, :expected_time, :start_time

  def post_initialize(args)
    ArgsValidator.validate(params: args,
                           required: [:event, :expected_time, :start_time],
                           required_alternatives: [:expected_laps, :comparison_time_points],
                           exclusive: [:event, :expected_time, :start_time, :expected_laps, :comparison_time_points,
                                       :effort_finder, :times_planner],
                           class: self.class)
    @event = args[:event]
    @expected_time = args[:expected_time]
    @start_time = args[:start_time]
    @expected_laps = args[:expected_laps]
    @comparison_time_points = args[:comparison_time_points] || event.time_points_through(expected_laps)
    @effort_finder = args[:effort_finder] || SimilarEffortFinder.new(time_point: finish_time_point,
                                                                     time_from_start: expected_time,
                                                                     finished: true)
    @times_planner = args[:times_planner] || SegmentTimesPlanner.new(expected_time: expected_time,
                                                                     event: event,
                                                                     time_points: comparison_time_points,
                                                                     similar_effort_ids: relevant_effort_ids,
                                                                     start_time: start_time)
  end

  def effort
    @effort ||= event.efforts.new
  end

  def lap_split_rows
    plan_times.present? ? super : []
  end

  def total_segment_time
    lap_split_rows.map(&:segment_time).compact.sum
  end

  def finish_time_from_start
    ordered_split_times.last.absolute_time - start_time
  end

  def relevant_efforts_count
    relevant_effort_ids.size
  end

  def event_years_analyzed
    relevant_events.map(&:start_time).sort.map(&:year).uniq
  end

  def relevant_events
    @relevant_events ||= effort_finder.events.to_a
  end

  def relevant_efforts
    @relevant_efforts ||= effort_finder.efforts.to_a
  end

  private

  attr_reader :expected_laps, :comparison_time_points, :effort_finder, :times_planner

  def ordered_split_times
    @ordered_split_times ||= plan_times_with_dummy.each_cons(2).map do |(_, prior_absolute_time), (time_point, absolute_time)|
      effort.split_times.new(time_point: time_point,
                             absolute_time: absolute_time,
                             segment_time: absolute_time && prior_absolute_time && absolute_time - prior_absolute_time)
    end
  end

  def plan_times
    @plan_times ||= times_planner.absolute_times(round_to: 1.minute).to_a
  end

  def plan_times_with_dummy
    [dummy_plan_time] + plan_times
  end

  def dummy_plan_time
    [nil, nil]
  end

  def finish_time_point
    time_points.last
  end

  def relevant_effort_ids
    effort_finder.effort_ids
  end

  def last_lap
    expected_laps || (comparison_time_points && comparison_time_points.map(&:lap).last) || 1
  end

  def time_zone
    event.home_time_zone
  end
end
