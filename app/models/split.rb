class Split < ActiveRecord::Base
  ...
  validates_uniqueness_of :kind, scope: :course_id, :if => 'split_is_start?',
                          :message => "only one start split permitted on a course"
  validates_uniqueness_of :kind, scope: :course_id, :if => 'split_is_finish?',
                          :message => "only one finish split permitted on a course"
  validates_numericality_of :distance_from_start, equal_to: 0, :if => 'split_is_start?',
                            :message => "the start split must have 0 distance from start"
  validates_numericality_of :vert_gain_from_start, equal_to: 0, :if => 'split_is_start?', allow_nil: true,
                            :message => "the start split vert_gain must be 0"
  validates_numericality_of :vert_loss_from_start, equal_to: 0, :if => 'split_is_start?', allow_nil: true,
                            :message => "the start split vert_loss must be 0"
  validates_numericality_of :distance_from_start, greater_than: 0, :unless => 'split_is_start?',
