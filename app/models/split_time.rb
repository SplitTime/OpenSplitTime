...
class SplitTime < ApplicationRecord
  ...
  scope :with_raw_time_matchers, -> { joins(:split, effort: :event).select("split_times.*, (events.start_time + efforts.start_offset * interval '1 second' + time_from_start * interval '1 second') as day_and_time, events.home_time_zone as event_home_zone, efforts.bib_number as bib_number, splits.parameterized_base_name as parameterized_split_name")}
