    ...
    query = <<-SQL
      ...
      FROM
        ...
        FROM
          (SELECT *,
              CASE 
                when laps_required = 0 then stopped  
                else
                  CASE
                    when laps_finished >= laps_required then true else false 
                  END 
              END
              AS finished
          FROM
            (SELECT #{effort_fields}, 
                true AS started,
                CASE when stopped_split_time_id is null then false else true end AS stopped,
