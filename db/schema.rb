...
ActiveRecord::Schema[7.0].define(version: 2022_11_04_031629) do
  ...
  create_view "best_effort_segments", sql_definition: <<-SQL
      ...
      ((begin_split.kind = 0) AND (end_split.kind = 1)) AS full_course
     FROM (((((efforts e
       JOIN effort_segments es ON ((es.effort_id = e.id)))
       JOIN splits begin_split ON ((begin_split.id = es.begin_split_id)))
       JOIN splits end_split ON ((end_split.id = es.end_split_id)))
