with applicants as (
     ...
     all_counts as (
                   select applicants.organization_id,
                          ...
                          coalesce(finish_year_count, 0)            as finish_ticket_count,
                          (coalesce(volunteer_year_count, 0)
                              + coalesce(vmulti_year_count, 0)) / 5 as volunteer_ticket_count,
                          coalesce(major_volunteer_year_count, 0)   as volunteer_major_ticket_count
                   from applicants
