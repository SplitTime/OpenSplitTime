with applicants as (
     ...
     dns_since_last_start_count as (
                   ...
                   where historical_facts.kind = 0
                     and historical_facts.year < 2026
     ...
     last_reset_year as (
                   ...
                   where efforts.finished
                      or (efforts.started and extract(year from events.scheduled_start_time) > 2022)
                   group by organization_id, person_id
                   ),
     ...
     finish_year_count as (
                   ...
                   from efforts
                        ...
                        join event_groups on event_groups.id = events.event_group_id
                   where efforts.finished
                     and extract(year from events.scheduled_start_time) < 2026
                   group by organization_id, person_id
                   ),
     ...
     volunteer_year_count as (
                   select historical_facts.organization_id, historical_facts.person_id, count(distinct historical_facts.year) as volunteer_year_count
                   from historical_facts
                   where kind in (1, 2)
                     and year < 2026
                   group by historical_facts.organization_id, historical_facts.person_id
     ...
     all_counts as (
                   select applicants.organization_id,
                          ...
                          ((coalesce(volunteer_year_count, 0)
                              + coalesce(vmulti_year_count, 0)) / 3)::int as volunteer_ticket_count,
