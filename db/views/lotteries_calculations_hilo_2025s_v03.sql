with applicants as (
     ...
     last_legacy_reset_year as (
                   select organization_id, person_id, max(year) as last_legacy_reset_year
                   from historical_facts
                   where kind = 16
     ...
     last_start_year as (
                   select organization_id, person_id, max(extract(year from events.scheduled_start_time)) as last_start_year
                   from efforts
                            join events on events.id = efforts.event_id
                            join event_groups on event_groups.id = events.event_group_id
                   where efforts.started
                     and events.course_id in (select id from high_lonesome_course_ids)
                   group by organization_id, person_id
                   ),

     last_reset_year as (
                   select organization_id, person_id, greatest(last_legacy_reset_year, last_start_year) as last_reset_year
                   from last_legacy_reset_year
                            natural left join last_start_year
                   ),
     ...
     volunteer_point_count as (
                   select historical_facts.organization_id,
                          ...
                          least(sum(historical_facts.quantity), 30) as volunteer_point_count
                   from historical_facts
                   where kind = 17
                     and year <= 2025
                   group by historical_facts.organization_id, historical_facts.person_id
     ...
     trail_work_hour_count as (
                   select historical_facts.organization_id,
                          historical_facts.person_id,
                          least(sum(historical_facts.quantity), 80) as trail_work_hour_count
                   from historical_facts
                   where kind = 18
                     and year <= 2025
                   group by historical_facts.organization_id, historical_facts.person_id
