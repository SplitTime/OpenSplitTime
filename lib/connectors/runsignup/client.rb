module Connectors
  module Runsignup
    class Client
      BASE_URL = "https://runsignup.com/Rest".freeze
      ...
      def get_participants(race_id, event_id, page)
        self.request = Connectors::Runsignup::Request::GetParticipants.new(race_id: race_id, event_id: event_id,
                                                                           page: page)
        make_request
      end
      ...
      def verify_credentials_present!
        return if runsignup_api_key.present? && runsignup_api_secret.present?
