  ...
  module Transformers
    module Async
      class EffortsWithTimesStrategy < Etl::Transformers::BaseTransformer
        DEFAULT_START_KEY = "start".freeze
        ...
        def add_missing_start_keys(proto_record)
          proto_record[guaranteed_start_key] = nil unless proto_record.key?(guaranteed_start_key)
        end
        ...
        def time_keys
          @time_keys ||= if finish_times_only?
                           [DEFAULT_START_KEY,
                            finish_key]
                         else
                           attribute_keys.elements_after(start_key, inclusive: true)
                         end
        end
        ...
        def missing_start_time?(proto_record)
          proto_record[guaranteed_start_key].blank? && time_keys.map do |key|
            proto_record[key].presence
          end.compact.present?
        end
