class Geodata
  ...
  SORTED_COUNTRIES = ALL_COUNTRIES.sort_by do |country|
    [PRIORITY_COUNTRY_CODES.index(country.code) || Float::INFINITY, country.name]
  end.freeze

  def self.serialize_country(country)
    { code: country.code,
      name: country.name,
      subregions: included_subregions(country).sort_by(&:name).to_h do |subregion|
