...
describe Api::V1::AidStationsController do
  ...
  describe '#show' do
    it 'returns a successful 200 response' do
      get :show, params: {id: aid_station}
  ...
  describe '#create' do
    it 'returns a successful json response' do
      post :create, params: {data: {type: 'aid_stations', attributes: {split_id: split.id, event_id: event.id} }}
      expect(response.body).to be_jsonapi_response_for('aid_stations')
      parsed_response = JSON.parse(response.body)
  ...
  describe '#update' do
    ...
    it 'returns a successful json response' do
      put :update, params: {id: aid_station, data: {type: 'aid_stations', attributes: attributes }}
      expect(response.body).to be_jsonapi_response_for('aid_stations')
      expect(response.status).to eq(200)
    end
    ...
    it 'returns an error if the aid_station does not exist' do
      put :update, params: {id: 0, data: {type: 'aid_stations', attributes: attributes }}
      parsed_response = JSON.parse(response.body)
      expect(parsed_response['errors']).to include(/not found/)
      expect(response.status).to eq(404)
    end
  ...
  describe '#destroy' do
    it 'returns a successful json response' do
      delete :destroy, params: {id: aid_station}
      expect(response.status).to eq(200)
    end
    ...
    it 'destroys the aid_station record' do
      ...
      expect(AidStation.all.count).to eq(1)
      delete :destroy, params: {id: test_aid_station}
      expect(AidStation.all.count).to eq(0)
    end
