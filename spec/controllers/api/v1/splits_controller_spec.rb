...
describe Api::V1::SplitsController do
  ...
  describe '#create' do
    it 'returns a successful json response' do
      post :create, data: {type: 'splits', attributes: {base_name: 'Test Split', course_id: course.id,
  ...
  describe '#update' do
    ...
    it 'returns a successful json response' do
      put :update, id: split, data: {type: 'splits', attributes: attributes }
      expect(response.body).to be_jsonapi_response_for('splits')
      expect(response.status).to eq(200)
    end
    ...
    it 'returns an error if the split does not exist' do
      put :update, id: 0, data: {type: 'splits', attributes: attributes }
      parsed_response = JSON.parse(response.body)
      expect(parsed_response['errors']).to include(/not found/)
      expect(response.status).to eq(404)
    end
