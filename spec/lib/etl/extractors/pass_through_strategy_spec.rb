RSpec.describe Etl::Extractors::PassThroughStrategy do
  subject { described_class.new(raw_data, options) }

  let(:options) { { model: :effort } }

  describe "#extract" do
    let(:raw_data) do
      [{ first_name: "Bjorn", lastName: "Borg", gender: "male" },
    ...
    it "returns an array of attribute rows in OpenStruct format with identical keys" do
      ...
      expect(attribute_rows.pluck(:first_name)).to eq(%w[Bjorn Charlie Lucy])
      expect(attribute_rows.pluck(:lastName)).to eq(%w[Borg Brown Pendergrast])
      expect(attribute_rows.pluck(:gender)).to eq(%w[male male female])
    end
  end
end
