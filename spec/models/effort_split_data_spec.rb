...
RSpec.describe EffortSplitData, type: :model do
  ...
  describe '#<=>' do
    ...
    context 'when the other object contains a later first day_and_time' do
      let(:days_and_times) { [DateTime.parse('2017-07-01 06:00:00'), DateTime.parse('2017-07-01 06:30:00')] }
      let(:days_and_times_other) { [DateTime.parse('2017-07-01 06:15:00'), DateTime.parse('2017-07-01 06:45:00')] }

      it 'sorts effort_split_data objects based on the first available time contained in days_and_times' do
        expect(effort_split_data_objects.sort).to eq([subject, other])
      end
    end

    context 'when the other object contains an earlier first day_and_time' do
      let(:days_and_times) { [DateTime.parse('2017-07-01 06:00:00'), DateTime.parse('2017-07-01 06:30:00')] }
    ...
    context 'when the other object contains no first day_and_time but an earlier second day_and_time' do
      let(:days_and_times) { [DateTime.parse('2017-07-01 06:00:00'), DateTime.parse('2017-07-01 06:30:00')] }
    ...
    context 'when the other object contains no day_and_time' do
      let(:days_and_times) { [DateTime.parse('2017-07-01 06:00:00'), DateTime.parse('2017-07-01 06:30:00')] }
