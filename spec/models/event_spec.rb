...
RSpec.describe Event, type: :model do
  ...
  describe '#time_points' do
    ...
    it 'returns an array of TimePoint objects ordered by lap, split distance, and bitkey' do
      ...
      expect(time_points[0]).to eq(TimePoint.new(111, 1, 1))
      expect(time_points[1]).to eq(TimePoint.new(102, 1, 1))
      expect(time_points[2]).to eq(TimePoint.new(102, 64, 1))
      expect(time_points[3]).to eq(TimePoint.new(103, 1, 1))
      expect(time_points[4]).to eq(TimePoint.new(103, 64, 1))
      expect(time_points[5]).to eq(TimePoint.new(112, 1, 1))
      expect(time_points[6]).to eq(TimePoint.new(111, 1, 2))
      expect(time_points[7]).to eq(TimePoint.new(102, 1, 2))
      expect(time_points[8]).to eq(TimePoint.new(102, 64, 2))
      expect(time_points[9]).to eq(TimePoint.new(103, 1, 2))
      expect(time_points[10]).to eq(TimePoint.new(103, 64, 2))
      expect(time_points[11]).to eq(TimePoint.new(112, 1, 2))
    end
  end
