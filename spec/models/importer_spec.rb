  ...
  describe 'initialization' do
    ...
    describe 'split_import' do
      ...
      it 'should import the splits correctly' do
        ...
        expect(Split.where(base_name: 'Start').first.distance_from_start).to eq(0)
        expect(Split.where(base_name: 'Tunnel').first.distance_from_start).to eq(46.6.miles.to.meters.to_i)
        expect(Split.where(base_name: 'Tunnel').count).to eq(1)
      end

      it 'should correctly set up sub_split_bitmaps for imported splits' do
        expect(Split.where(base_name: 'Start').first.sub_split_bitmap).to eq(1)
        expect(Split.where(base_name: 'Tunnel').first.sub_split_bitmap).to eq(65)
      end

      it 'should correctly determine start status' do
        expect(Split.start.count).to eq(1)
        expect(Split.where(base_name: 'Start').first.kind).to eq('start')
      end

      it 'should correctly determine intermediate status' do
        expect(Split.intermediate.count).to eq(4)
        expect(Split.where(base_name: 'Ridgeline').first.kind).to eq('intermediate')
      end

      it 'should correctly determine finish status' do
        expect(Split.finish.count).to eq(1)
        expect(Split.where(base_name: 'Finish').first.kind).to eq('finish')
      end
    ...
    describe 'effort_import' do
      ...
      it 'should return a failure array containing invalid rows' do
        ...
        expect(effort_importer.effort_failure_array.first.first).to eq('NoLastName')
        expect(effort_importer.effort_failure_array.last).to eq('NoFirstName')
      end
      ...
      it 'should correctly assign country codes' do
        ...
        expect(Effort.where(bib_number: 40).first.country_code).to eq('GB')
      ...
      it 'should correctly assign genders' do
        ...
        expect(Effort.where(bib_number: 143).first.gender).to eq('female')
        expect(Effort.where(bib_number: 32).first.gender).to eq('male')
        expect(Effort.where(bib_number: 119).first.gender).to eq('female')
      end

      it 'should correctly determine dropped_split_id' do
        split1 = Split.where(base_name: 'Tunnel').first
        split2 = Split.where(base_name: 'Mountain Town').first
      ...
      it 'should correctly set split_time times from start' do
        split1 = Split.where(base_name: 'Ridgeline').first
        split2 = Split.where(base_name: 'Mountain Town').first
        split3 = Split.where(base_name: 'Tunnel').first
        split4 = Split.where(base_name: 'Finish').first
      ...
      it 'should not create a split_time where no time is provided' do
        split1 = Split.where(base_name: 'Ridgeline').first
        split2 = Split.where(base_name: 'Tunnel').first
        split3 = Split.where(base_name: 'Finish').first
