...
RSpec.describe SplitTime, kind: :model do
  ...
  it "should be valid when created with an effort, a split, a sub_split, and a time_from_start" do
    SplitTime.create!(effort: @effort, split: @split, sub_split_key: SubSplit::IN_KEY, time_from_start: 30000)

    expect(SplitTime.all.count).to eq(1)
    expect(SplitTime.first.effort).to eq(@effort)
    expect(SplitTime.first.split).to eq(@split)
    expect(SplitTime.first.sub_split_key).to eq(SubSplit::IN_KEY)
  ...
  it "should be invalid without an effort" do
    ...
    split_time = SplitTime.new(effort: nil, split: split, sub_split_key: SubSplit::IN_KEY, time_from_start: 0)
  ...
  it "should be invalid without a split_id" do
    split_time = SplitTime.new(effort: @effort, split: nil, sub_split_key: SubSplit::IN_KEY, time_from_start: 0)
    expect(split_time).not_to be_valid
    expect(split_time.errors[:split_id]).to include("can't be blank")
  end

  it "should be invalid without a sub_split_key" do
    split = Split.new(course: @course, location: @location1, base_name: 'Race Start', distance_from_start: 0, kind: 0)
    split_time = SplitTime.new(effort: @effort, split: split, sub_split_key: nil, time_from_start: 0)
    expect(split_time).not_to be_valid
  ...
  it "should be invalid without a time_from_start" do
    split = Split.create!(course: @course, location: @location1, base_name: 'Race Start', distance_from_start: 0, kind: 0)
    split_time = SplitTime.new(effort: @effort, split: split, sub_split_key: SubSplit::IN_KEY, time_from_start: nil)
    expect(split_time).not_to be_valid
  ...
  it "should not allow more than one of a given split_id/sub_split combination within an effort" do
    ...
    SplitTime.create!(effort: @effort, split: split, sub_split_key: SubSplit::IN_KEY, time_from_start: 10000)
    split_time = SplitTime.new(effort: @effort, split: split, sub_split_key: SubSplit::IN_KEY, time_from_start: 11000)
    expect(split_time).not_to be_valid
  ...
  it "should allow within an effort one of a given split_id for each sub_split" do
    split = Split.create!(course: @course, location: @location1, base_name: 'Aid Station', distance_from_start: 10000, kind: 2)
    SplitTime.create!(effort: @effort, split: split, sub_split_key: SubSplit::IN_KEY, time_from_start: 10000)
    split_time = SplitTime.new(effort: @effort, split: split, sub_split_key: SubSplit::OUT_KEY, time_from_start: 11000)
  ...
  it "should allow multiple of a given split_id/sub_split combination within different efforts" do
    ...
    SplitTime.create!(effort: @effort, split: split, sub_split_key: SubSplit::IN_KEY, time_from_start: 10000)
    split_time1 = SplitTime.new(effort: effort2, split: split, sub_split_key: SubSplit::IN_KEY, time_from_start: 11000)
    split_time2 = SplitTime.new(effort: effort3, split: split, sub_split_key: SubSplit::IN_KEY, time_from_start: 12000)
    split_time3 = SplitTime.new(effort: effort4, split: split, sub_split_key: SubSplit::IN_KEY, time_from_start: 13000)
