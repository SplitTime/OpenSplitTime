...
RSpec.describe SplitTime, kind: :model do
  ...
  it 'is valid when created with an effort, a split, a sub_split, a time_from_start, and a lap' do
    ...
    expect(SplitTime.first.sub_split_bitkey).to eq(in_bitkey)
    expect(SplitTime.first.time_from_start).to eq(30000)
  end

  it 'is invalid without an effort' do
    split_time = SplitTime.new(effort: nil, split: intermediate_split, bitkey: in_bitkey, time_from_start: 0, lap: 1)
  ...
  it 'is invalid without a split_id' do
    split_time = SplitTime.new(effort: effort, split: nil, bitkey: in_bitkey, time_from_start: 0, lap: 1)
    expect(split_time).not_to be_valid
  ...
  it 'is invalid without a time_from_start' do
    split_time = SplitTime.new(effort: effort, split: intermediate_split, bitkey: in_bitkey, time_from_start: nil, lap: 1)
    expect(split_time).not_to be_valid
    expect(split_time.errors[:time_from_start]).to include("can't be blank")
  end

  it 'is invalid without a lap' do
    split_time = SplitTime.new(effort: effort, split: intermediate_split, bitkey: in_bitkey, time_from_start: 0, lap: nil)
    expect(split_time).not_to be_valid
  ...
  it 'allows multiple of a given split_id/sub_split/lap combination within different efforts' do
    ...
    SplitTime.create!(effort: effort, split: intermediate_split, bitkey: in_bitkey, time_from_start: 10000, lap: 1)
    split_time1 = SplitTime.new(effort: effort2, split: intermediate_split, bitkey: in_bitkey, time_from_start: 11000, lap: 1)
    split_time2 = SplitTime.new(effort: effort3, split: intermediate_split, bitkey: in_bitkey, time_from_start: 12000, lap: 1)
    split_time3 = SplitTime.new(effort: effort4, split: intermediate_split, bitkey: in_bitkey, time_from_start: 13000, lap: 1)
  ...
  describe '#day_and_time' do
    ...
    it 'returns a day and time equal to event start_time if time_from_start and start_offset are zero' do
      split_time = SplitTime.new(effort: effort, split: intermediate_split, bitkey: in_bitkey, time_from_start: 0)
    ...
    it 'returns a day and time equal to event start_time plus time_from_start when start_offset is zero' do
      split_time = SplitTime.new(effort: effort, split: intermediate_split, bitkey: in_bitkey, time_from_start: 3600)
    ...
    it 'returns correct day and time when time_from_start is greater than 24 hours' do
      split_time = SplitTime.new(effort: effort, split: intermediate_split, bitkey: in_bitkey, time_from_start: 100000)
    ...
    it 'returns correct day and time when time_from_start is greater than 100 hours' do
      split_time = SplitTime.new(effort: effort, split: intermediate_split, bitkey: in_bitkey, time_from_start: 500000)
    ...
    it 'returns correct day and time when start_offset is greater than zero' do
      ...
      split_time = SplitTime.new(effort: effort1, split: intermediate_split, bitkey: in_bitkey, time_from_start: 7200)
  ...
  describe '#day_and_time=' do
    it 'sets time_from_start to nil if passed a nil value' do
      split_time = SplitTime.new(effort: effort, split: intermediate_split, bitkey: in_bitkey, time_from_start: 1000)
      split_time.day_and_time = nil
      expect(split_time.time_from_start).to be_nil
    end

    it 'sets time_from_start to zero if passed the event start_time when start_offset is zero' do
      split_time = SplitTime.new(effort: effort, split: intermediate_split, bitkey: in_bitkey)
    ...
    it 'sets time_from_start to zero if passed the event start_time plus the effort start offset' do
      effort1 = Effort.create!(event: event, first_name: 'David', last_name: 'Goliath', gender: 'male', start_offset: 3600)
      split_time = SplitTime.new(effort: effort1, split: intermediate_split, bitkey: in_bitkey)
  ...
  describe '#time_point' do
    it 'returns split_id, sub_split_bitkey, and lap in a TimePoint struct' do
      split_time = SplitTime.new(split_id: 101, bitkey: in_bitkey, lap: 2)
