...
RSpec.describe SplitTime, kind: :model do
  ...
  describe "validations" do
    describe "for validations that do not depend on existing records in the database" do
      ...
      context "when no effort exists" do
        ...
        it "is invalid" do
          ...
          expect(split_time.errors[:effort]).to include("must exist")
        end
      end
      ...
      context "when no split exists" do
        ...
        it "is invalid" do
          expect(split_time).not_to be_valid
          expect(split_time.errors[:split]).to include("must exist")
        end
      end
    ...
    describe "for validations that rely on existing records in the database" do
  ...
  describe "before_update" do
    ...
    context "when matching_raw_time_id is set and exists" do
      it_behaves_like "conforms and matches the proposed time"
    end

    context "when matching_raw_time_id is not found" do
      let(:proposed_time_id) { 0 }
      it_behaves_like "does not conform or match the proposed time"
    end
  ...
  describe "before_save" do
    context "when the split time exists" do
      subject { split_times(:hardrock_2016_brinda_fisher_telluride_out_1) }

      context "when elapsed seconds is not already set" do
      ...
      context "when elapsed seconds is already set" do
        it "sets elapsed seconds based on absolute time" do
    ...
    context "when the split time is newly created" do
    ...
    context "when the split time is a start" do
      ...
      it "sets elapsed seconds for all split times for the effort" do
        ...
        expect(effort.split_times.pluck(:elapsed_seconds)).to contain_exactly(0.0, 43_020.0, 43_200.0, 68_160.0, 69_900.0, 97_500.0, 98_340.0)

        subject.update(absolute_time: "2016-07-15 12:00:00")
        expect(effort.split_times.pluck(:elapsed_seconds)).to contain_exactly(0.0, 39_420.0, 39_600.0, 64_560.0, 66_300.0, 93_900.0, 94_740.0)
      end
    end
  end
  ...
  describe "before destroy" do
    ...
    context "when the starting split time has no other split times" do
  ...
  describe "virtual time attributes" do
    ...
    describe "getters" do
      ...
      describe "#elapsed time" do
        ...
        context "when with_fractionals: false is used" do
          let(:absolute_time) { effort_start_time + 4530.55 }
    ...
    describe "setters" do
      ...
      describe "#military_time=" do
        ...
        context "when the split time is not affected by Daylight Savings" do
        ...
        context "when the split time occurs on a Daylight Savings Time switch day" do
  ...
  describe "#sub_split" do
    it "returns a SubSplit object with split_id and sub_split_bitkey" do
      split_time = described_class.new(split_id: 101, bitkey: in_bitkey)
  ...
  describe "#sub_split=" do
    it "sets both split_id and sub_split_bitkey" do
      split_time = described_class.new(sub_split: SubSplit.new(101, in_bitkey))
  ...
  describe "#time_point" do
    it "returns lap, split_id, and sub_split_bitkey in a TimePoint struct" do
      split_time = described_class.new(lap: 2, split_id: 101, bitkey: in_bitkey)
  ...
  describe "#time_point=" do
    it "sets lap, split_id, and sub_split_bitkey" do
      ...
      split_time = described_class.new(time_point: time_point)
      expect(split_time.split_id).to eq(101)
      expect(split_time.bitkey).to eq(1)
  ...
  describe "#lap_split" do
    it "returns a LapSplit object" do
      lap = 2
      split_time = described_class.new(lap: lap, split_id: 101, bitkey: in_bitkey)
  ...
  describe "#effort_lap_key" do
    it "returns effort_id and lap in an EffortLapKey struct" do
      split_time = described_class.new(effort_id: 101, lap: 2)
  ...
  describe "#split_name" do
    ...
    it "does not indicate the lap even when available" do
      ...
      st = described_class.new(split: split, bitkey: in_bitkey, lap: 1)
      expected = "Aid 1"
      expect(st.split_name).to eq(expected)
    end

    context "when the split has multiple sub_splits" do
      it "returns the name of the split with sub_split extension" do
        split = Split.new(base_name: "Aid 1", sub_split_bitmap: 65)
        st = described_class.new(split: split, bitkey: in_bitkey)
        expected = "Aid 1 In"
        expect(st.split_name).to eq(expected)

        st = described_class.new(split: split, bitkey: out_bitkey)
        expected = "Aid 1 Out"
        expect(st.split_name).to eq(expected)
      end
    end

    context "when the split has a single sub_split" do
  ...
  describe "#split_name_with_lap" do
    ...
    context "when the split has multiple sub_splits" do
      it "returns the name of the split with sub_split extension and a lap indication" do
        ...
        st = described_class.new(split: split, bitkey: out_bitkey, lap: 1)
