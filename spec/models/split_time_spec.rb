...
RSpec.describe SplitTime, kind: :model do
  ...
  before :each do
    ...
    @split = Split.create!(course: @course, location: @location1, base_name: 'Hopeless Outbound', sub_split_bitmap: 3, distance_from_start: 50000, kind: 2)
    
  end
  it "should be valid when created with an effort, a split, a sub_split, and a time_from_start" do
    SplitTime.create!(effort: @effort, split: @split, sub_split_bitkey: SubSplit::IN_BITKEY, time_from_start: 30000)
  ...
  it "should be invalid without an effort" do
    ...
    split_time = SplitTime.new(effort: nil, split: split, sub_split_bitkey: SubSplit::IN_BITKEY, time_from_start: 0)
  ...
  it "should be invalid without a split_id" do
    split_time = SplitTime.new(effort: @effort, split: nil, sub_split_bitkey: SubSplit::IN_BITKEY, time_from_start: 0)
    expect(split_time).not_to be_valid
  ...
  it "should be invalid without a sub_split_bitkey" do
    ...
    split_time = SplitTime.new(effort: @effort, split: split, sub_split_bitkey: nil, time_from_start: 0)
    expect(split_time).not_to be_valid
  ...
  it "should be invalid without a time_from_start" do
    split = Split.create!(course: @course, location: @location1, base_name: 'Race Start', distance_from_start: 0, kind: 0)
    split_time = SplitTime.new(effort: @effort, split: split, sub_split_bitkey: SubSplit::IN_BITKEY, time_from_start: nil)
    expect(split_time).not_to be_valid
  ...
  it "should allow within an effort one of a given split_id for each sub_split" do
    split = Split.create!(course: @course, location: @location1, base_name: 'Aid Station', distance_from_start: 10000, kind: 2)
    SplitTime.create!(effort: @effort, split: split, sub_split_bitkey: SubSplit::IN_BITKEY, time_from_start: 10000)
    split_time = SplitTime.new(effort: @effort, split: split, sub_split_bitkey: SubSplit::OUT_BITKEY, time_from_start: 11000)
  ...
  it "should allow multiple of a given split_id/sub_split combination within different efforts" do
    ...
    SplitTime.create!(effort: @effort, split: split, sub_split_bitkey: SubSplit::IN_BITKEY, time_from_start: 10000)
    split_time1 = SplitTime.new(effort: effort2, split: split, sub_split_bitkey: SubSplit::IN_BITKEY, time_from_start: 11000)
    split_time2 = SplitTime.new(effort: effort3, split: split, sub_split_bitkey: SubSplit::IN_BITKEY, time_from_start: 12000)
    split_time3 = SplitTime.new(effort: effort4, split: split, sub_split_bitkey: SubSplit::IN_BITKEY, time_from_start: 13000)
