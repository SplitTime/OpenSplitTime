  ...
  describe '#transform' do
    ...
    context 'when event is present and splits count matches split fields count' do
      ...
      it 'splits full names into first names and last names' do
        expect(proto_records.map { |pr| pr[:first_name] }).to eq(%w(Jatest Tatest Castest Mictest N.n.))
        expect(proto_records.map { |pr| pr[:last_name] }).to eq(%w(Schtest Notest Pertest Hintest 62))
      end
