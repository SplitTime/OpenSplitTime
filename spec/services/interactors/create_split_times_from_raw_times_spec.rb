...
RSpec.describe Interactors::CreateSplitTimesFromRawTimes do
  ...
  let(:time_zone) { ActiveSupport::TimeZone[event_group.home_time_zone] }
  let(:time_1) { time_zone.parse('2018-02-10 09:00:00') }
  let(:time_2) { time_zone.parse('2018-02-10 10:00:00') }

  let(:raw_time_1) { create(:raw_time, bib_number: effort.bib_number, event_group: event_group, split_name: split_2.base_name, bitkey: out_bitkey, absolute_time: time_1) }
  let(:raw_time_2) { create(:raw_time, bib_number: effort.bib_number, event_group: event_group, split_name: split_3.base_name, bitkey: in_bitkey, absolute_time: time_2) }
  ...
  describe '#perform!' do
    context 'when raw_times do not match existing split_times' do
      it 'creates new split_times from the given raw_times' do
        ...
        expect(new_split_times.map(&:military_time)).to match_array(raw_times.map { |lt| lt.military_time(event_group.home_time_zone) })
      end
    ...
    context 'when some raw_times match existing split_times' do
      ...
      let(:matching_raw_time) { create(:raw_time, bib_number: effort.bib_number, event_group: event_group, split: split_1, bitkey: in_bitkey, absolute_time: matching_time) }
      ...
      it 'creates new split_times for unmatched raw_times only' do
        ...
        expect(new_split_times.map(&:military_time)).to match_array(unmatched_raw_times.map { |lt| lt.military_time(event_group.home_time_zone) })
      end
    end
  end
end
